...
if __name__ == '__main__':
  ...
  for SELECTED_COLUMN in ["pm25_median"]:#, "so2_median", "pm10_median", "no2_median", "co_median"]:
      ...
      for model_name in ['RNN', 'LSTM', 'TransLSTM', 'Transformer', 'CosFormer', 'CosSquareFormer']:
          ...
          elif model_name == 'CosFormer':
              model = Transformer(num_layers=6, D=16, H=10, hidden_mlp_dim=32, inp_features=11, out_features=1, dropout_rate=0.1, attention_type='cosine', SL=SEQ_LENGTH).to(device) # cosine_square, cosine, regular # 6L, 12
          elif model_name == 'CosSquareFormer':
              model = Transformer(num_layers=6, D=16, H=10, hidden_mlp_dim=32, inp_features=11, out_features=1, dropout_rate=0.1, attention_type='cosine_square', SL=SEQ_LENGTH).to(device) # cosine_square, cosine, regular # 6L, 12H
          else :
