...
# SET HERE
# SELECTED_COLUMN = "median_co" # ["median_pm25", "median_o3", "median_so2", "median_no2", "median_pm10", "median_co"]:
SEQ_LENGTH = 7
...
class CityDataForecast(torch.utils.data.Dataset):
  ...
  def get_idx_data(self, idx):
    ...
    city = cities_list[self.valid_day_idx]
    _df = self.dataset[city]
  ...
  def __len__(self):
    ...
    return len(train_set) - (SEQ_LENGTH - 1)
...
def train(sampleLoader, val_loader, model, model_name, SELECTED_COLUMN, mask=False, LUR=False, l1=False, l2=False):
    ...
    for epoch in range(1, n_epochs + 1):
        ...
        if eval_mse < best_mse:
            ...
            best_mse = eval_mse
            mape = eval_mape
            torch.save(best_model.state_dict(), "./save_US_integ/"+SELECTED_COLUMN+"/"+model_name+"_"+city+"{}.pth".format(SEQ_LENGTH))
...
if __name__ == '__main__':
    ...
    for SELECTED_COLUMN in ["median_pm25", "median_so2", "median_pm10", "median_no2", "median_co", "median_o3"]:
