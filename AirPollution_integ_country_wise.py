...
# 'CA', 'CN', 'DE', 'ES', 'FR', 'GB', 'IN', 'IR', 'JP', 'KR', 'RO', 'TR', 'US'
Use_contry = 'IR'
if DROP_ONEHOT:
    INPUT_DIM = 8
else:
...
def train(sampleLoader, val_loader, model, model_name, SELECTED_COLUMN, mask=False, LUR=False, l1=False, l2=False):
    ...
    for epoch in range(1, n_epochs + 1):
        ...
        if eval_mse < best_mse:
            ...
            best_mse = eval_mse
            mape = eval_mape
            torch.save(best_model.state_dict(), "./save_KCC/"+SELECTED_COLUMN+"/"+model_name+"_{}_{}.pth".format(Use_contry, SEQ_LENGTH))
...
if __name__ == '__main__':
    ...
    df = pd.read_csv("./data/"+Use_contry+"2019to2023.csv")
    ...
    # for SELECTED_COLUMN in ["median_no2"]: # "median_pm25","median_pm25", "median_pm10", 

    #     train_data = CityDataEstimatePollutant(SELECTED_COLUMN, "train")
    #     val_data = CityDataEstimatePollutant(SELECTED_COLUMN, "test")
    #     sampleLoader = DataLoader(train_data, 1024, shuffle=True, num_workers=4, worker_init_fn=seed_worker, generator=g)
    #     val_loader = DataLoader(val_data, 4096, shuffle=False, num_workers=4, worker_init_fn=seed_worker, generator=g)
