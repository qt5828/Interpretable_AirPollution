...
class CityDataForecast(torch.utils.data.Dataset):
  def __init__(self, selected_column, split, train_set, test_set):
    self.split = split
    if split == "train":
...
def train(sampleLoader, val_loader, model, model_name, SELECTED_COLUMN, mask=False, LUR=False, l1=False, l2=False):
    ...
    model.to(device)
    print(next(model.parameters()).is_cuda, ": iteration ce")
    ...
    for epoch in range(1, n_epochs + 1):
        ...
        if eval_mse < best_mse:
            ...
            best_mse = eval_mse
            mape = eval_mape
            torch.save(best_model.state_dict(), "./save_KCC_ours/"+SELECTED_COLUMN+"/"+model_name+"_wo_C_{}.pth".format(SEQ_LENGTH))
...
if __name__ == '__main__':
    ...
    for SELECTED_COLUMN in ["median_pm25","median_pm10","median_no2"]:  # ["median_pm25", "median_so2", "median_pm10", "median_no2", "median_co", "median_o3"]

        train_data = CityDataForecast(SELECTED_COLUMN, "train", train_set, test_set)
        val_data = CityDataForecast(SELECTED_COLUMN, "test", train_set, test_set)
        sampleLoader = DataLoader(train_data, 512, shuffle=True, num_workers=4, worker_init_fn=seed_worker, generator=g)
        ...
        # Transmodel = DDP(Transmodel, delay_allreduce=True)
        ...
        # TransCosSquare = DDP(TransCosSquare, delay_allreduce=True)
        train(sampleLoader, val_loader, TransCosSquare, "CosSquareFormer", SELECTED_COLUMN, mask=True)

    
    # for SELECTED_COLUMN in ["median_pm25","median_pm10","median_no2"]:

    #     train_data = CityDataEstimatePollutant(SELECTED_COLUMN, "train", train_set, test_set)
    #     val_data = CityDataEstimatePollutant(SELECTED_COLUMN, "test", train_set, test_set)
    #     sampleLoader = DataLoader(train_data, 512, shuffle=True, num_workers=4, worker_init_fn=seed_worker, generator=g)
    #     val_loader = DataLoader(val_data, 4096, shuffle=False, num_workers=4, worker_init_fn=seed_worker, generator=g)
        
    #     OLS = LinearRegression(input_dim=INPUT_DIM).to(device)
    #     # OLS = DDP(OLS, delay_allreduce=True)
    #     train(sampleLoader, val_loader, OLS, "OLS", SELECTED_COLUMN, LUR=True)
    #     LASSO = LinearRegression(input_dim=INPUT_DIM).to(device)
    #     # LASSO = DDP(LASSO, delay_allreduce=True)
    #     train(sampleLoader, val_loader, LASSO, "LASSO", SELECTED_COLUMN, LUR=True, l1=True)
    #     Ridge = LinearRegression(input_dim=INPUT_DIM).to(device)
    #     # Ridge = DDP(Ridge, delay_allreduce=True)
    #     train(sampleLoader, val_loader, Ridge, "Ridge", SELECTED_COLUMN, LUR=True, l2=True)
    #     Elastic = LinearRegression(input_dim=INPUT_DIM).to(device)
    #     # Elastic = DDP(Elastic, delay_allreduce=True)
