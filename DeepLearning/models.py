...
class MultiHeadAttentionCosformer(nn.Module):
    '''Multi-head self-attention module'''
    def __init__(self, D, H):
        super(MultiHeadAttentionCosformer, self).__init__()
        self.H = H # number of heads
        self.D = D # dimension
...
class MultiHeadAttentionCosSquareformer(nn.Module):
    '''Multi-head self-attention module'''
    def __init__(self, D, H):
        super(MultiHeadAttentionCosSquareformer, self).__init__()
        self.H = H # number of heads
        self.D = D # dimension
...
def positional_encoding(D, position=20, dim=3, device=("cuda" if torch.cuda.is_available() else "cpu") ):
...
class TransformerLayer(nn.Module):
    def __init__(self, D, H, hidden_mlp_dim, dropout_rate, attention_type='cosine_square'):
        ...
        if attention_type == 'cosine':
          self.mha = MultiHeadAttentionCosformer(D, H)
        elif attention_type == 'cosine_square':
          self.mha = MultiHeadAttentionCosSquareformer(D, H)
        else:
